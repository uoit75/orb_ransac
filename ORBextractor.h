...
protected:
    ...
    int nfeatures = 1000;			                    ///<整个图像金字塔中，要提取的特征点数目
    double scaleFactor = 1.2;		                    ///<图像金字塔层与层之间的缩放因子
    int nlevels = 8;			                    ///<图像金字塔的层数
    int iniThFAST = 20;			                    ///<初始的FAST响应值阈值
    int minThFAST = 7;			                    ///<最小的FAST响应值阈值

    std::vector<int> mnFeaturesPerLevel = {61, 73, 87, 105, 126, 151, 181, 216};		///<分配到每层图像中，要提取的特征点数目

    std::vector<int> umax;	                    ///<计算特征点方向的时候，有个圆形的图像区域，这个vector中存储了每行u轴的边界（四分之一，其他部分通过对称获得）

    std::vector<float> mvScaleFactor = {1, 1.2, 1.44, 1.728, 2.074, 2.488, 2.986, 3.583};		    ///<每层图像的缩放因子
    std::vector<float> mvInvScaleFactor = {1, 0.833, 0.694, 0.579, 0.482, 0.402, 0.335, 0.2791};        ///<以及每层缩放因子的倒数
    std::vector<float> mvLevelSigma2 = {1, 1.44, 2.074, 2.986, 4.300, 6.190, 8.916, 12.838};		    ///<存储每层的sigma^2,即上面每层图像相对于底层图像缩放倍数的平方
    std::vector<float> mvInvLevelSigma2 = {1, 0.694, 0.482, 0.335, 0.233, 0.162, 0.112, 0.078};	    ///<sigma平方的倒数
};
