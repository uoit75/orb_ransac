 ...
 *  
 */
Frame::Frame(const cv::Mat &imLeft, const cv::Mat &imRight, ORBextractor* extractorLeft, ORBextractor* extractorRight, cv::Mat &K, const float &bf, const float &thDepth)
    :mpORBextractorLeft(extractorLeft),mpORBextractorRight(extractorRight), mK(K.clone()), mbf(bf), mThDepth(thDepth)
{
    // Step 1 帧的ID 自增
    mnId=nNextId++;
    // Step 2 计算图像金字塔的参数 
    ...
    ComputeStereoMatches(imLeft, imRight);
    // // 初始化本帧的地图点
...
{   
    ...
    for(int i=0; i<N; i++)
    {
        ...
        const cv::KeyPoint &kp1 = keypoints_L[i];
        const cv::KeyPoint &kp2 = keypoints_R[i];
 ...
 */
void Frame::ComputeStereoMatches(const cv::Mat &imLeft, const cv::Mat &imRight)
{
    ...
    for(int iR = 0; iR < Nr; iR++) {
        ...
        const float r = 8.0f * mvScaleFactors[mvKeysRight[iR].octave];
    ...
    for(int iL=0; iL<N; iL++) 
    {
        ...
        if(bestDist<thOrbDist) {
            ...
            if(disparity>=minD && disparity<maxD) {
                ...
                keypoints_L.push_back(kpL);
                keypoints_R.push_back(mvKeysRight[bestIdxR]);
                cv::DMatch match;
                match.queryIdx = num_keypoint;
                match.trainIdx = num_keypoint;
    ...
    for(int i=vDistIdx.size()-1;i>=0;i--) {
        if(vDistIdx[i].first<thDist)
        {
            ...
            keypoints_L.erase(keypoints_L.begin()+i);
            keypoints_R.erase(keypoints_R.begin()+i);
    ...
    //绘制图像
    cv::Mat img_match;
    cv::drawMatches(imLeft, keypoints_L, imRight, keypoints_R, good_matches, img_match);
    cv::imshow("Matched Features", img_match);
    //cv::waitKey(0);
    std::string filename_good = "good_matches" + std::to_string(mnId) + ".png";
    cv::imwrite(filename_good, img_match);
    ...
    int mMaxIterations = 500;
    //mvSets用于保存每次迭代时所使用的向量
    std::vector<std::vector<size_t>>mvSets = vector< vector<size_t> >(mMaxIterations,vector<size_t>(8,0));
    ...
    for(int it=0; it<mMaxIterations; it++)
    {
        ...
        for(size_t j=0; j<8; j++)
        {
            ...
            int idx = vAvailableIndices[random_index];
            ...
            // vAvailableIndices[random_index] = vAvailableIndices.back();
	...
	// 记录各自的归一化矩阵
    cv::Mat T1, T2;
    Normalize(keypoints_L,vPn1, T1);
    Normalize(keypoints_R,vPn2, T2);
    ...
    for(int it=0; it<mMaxIterations; it++)
    {
        ...
        for(size_t j=0; j<8; j++)
        {
            ...
            vPn1i[j] = vPn1[good_matches[idx].queryIdx];    //first存储在参考帧1中的特征点索引
            vPn2i[j] = vPn2[good_matches[idx].trainIdx];   //second存储在参考帧1中的特征点索引
        }//读取8对特征点的归一化之后的坐标
        ...
        currentScore = CheckHomography(H21i, H12i, 			//输入，单应矩阵的计算结果
									   ...
									   2,                   //TODO  测量误差，在Initializer类对象构造的时候，由外部给定的
                                       keypoints_L,
                                       keypoints_R,
                                       good_matches);				
...
cout<<score<<endl;
// cout<<vbMatchesInliers.size()<<endl;
// cout<<good_matches.size()<<endl;
    ...
    for(int i=0; i<vbMatchesInliers.size(); i++)
    {
        ...
        {
        cv::DMatch match_ransac;
        match_ransac.queryIdx = num_ransac;
        match_ransac.trainIdx = num_ransac;
        good_matches_ransac.push_back(match_ransac);
        num_ransac++;
        }
    }

    //绘制图像
    cv::Mat img_match_ransac;
    cout<<keypoints_L.size()<<endl;
    cout<<keypoints_R.size()<<endl;
    cout<<good_matches_ransac.size()<<endl;
    cv::drawMatches(imLeft, keypoints_L, imRight, keypoints_R, good_matches_ransac, img_match_ransac);
    cv::imshow("Matched Features", img_match_ransac);
    //cv::waitKey(0);
    std::string filename_good_ransac = "good_matches_ransac" + std::to_string(mnId) + ".png";
    cv::imwrite(filename_good_ransac, img_match_ransac);
